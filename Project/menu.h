...
public:
	...
	bool getIgnoreDirectories() {
		cout << GREEN_BACK << WHITE_TEXT << "Press button" << endl << "1- to ignore directories" << endl << "2- not to ignore directories" << RESET_COLOR << endl;
		int button;
		cin >> button;
		if (button == 1) {
			return true;
			...
			cout << RED_BACK << WHITE_TEXT << "Wrong button" << RESET_COLOR << endl;;
			getIgnoreDirectories();
		}
	}

	void getAvaliableTypes() {
		cout<< GREEN_BACK << WHITE_TEXT << "Press button" << endl << "1- to add to already possible types" << endl << "2- to add your own types" <<endl<<"3- to use default types"<< RESET_COLOR <<endl;
		int button;
		cin >> button;
		if (button == 1) {
			...
			cout<<endl;
			do  {
				cout << GREEN_BACK << WHITE_TEXT << "Enter type to add: " << RESET_COLOR;
				string type;
				getline(cin >> ws, type);
				fileCollector.addFileType(type);
				cout << GREEN_BACK << WHITE_TEXT <<"Press button" << endl << "1-to continue adding types" << endl << "Random key- to stop adding types" << RESET_COLOR << endl;
				cin>> decision;
			} while (decision == 1);
		}
		else if (button == 2) {
	...
	void getRootDir() {
		cout << GREEN_BACK << WHITE_TEXT << "Enter root dir (path):"<< RESET_COLOR << endl;
		string rootDir;
		getline(cin, rootDir);
	...
	void filesToDelete() {
		...
		cout << GREEN_BACK << WHITE_TEXT << "Please enter indexes of original files whose copies you want to delete\n(Enter -1 to delete all)\n" << RESET_COLOR;
		vector<int> inputs;
		...
		for (int i : inputs) {
			...
			cout << GREEN_TEXT << "Copies of " << file << " were successfully deleted" << RESET_COLOR << endl;
